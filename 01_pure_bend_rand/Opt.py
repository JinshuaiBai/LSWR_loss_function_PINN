...
class Opt_lbfgsb:
    ...
    def __init__(self, pinn, x_train, y_train, dx, h, ar, factr=10, pgtol=1e-10, m=50, maxls=50, maxiter=20000):
        """
        =================================================================================================================================
    ...
    def Loss_grad(self, x, y):
        ...
        with tf.GradientTape() as g:
            l1 = self.h * (tf.reduce_sum(self.ar * tf.square(self.pinn(x)[0])) + \
                tf.reduce_sum(self.ar * tf.square(self.pinn(x)[1])))
            l2 = (self.dx * (tf.reduce_sum(tf.square(self.pinn(x)[2]-y[0])) + \
                ...
                tf.reduce_sum(tf.square(self.pinn(x)[5]-y[3]))) + \
                self.dx * (tf.reduce_sum(tf.square(self.pinn(x)[7]-y[5]))) + \
                self.dx * (tf.reduce_sum(tf.square(self.pinn(x)[8]-y[6]))) + \
                self.dx * (tf.reduce_sum(tf.square(self.pinn(x)[9]-y[7]))))
            l3 = 0 * tf.reduce_sum(self.pinn(x)[10])
            loss = l1 + l2 + l3
